...
func TestHandleDoubleSign(t *testing.T) {
	...
	ctx = ctx.WithBlockHeader(abci.Header{Time: 1 + keeper.params.MaxEvidenceAge(ctx)})
...
func TestHandleAbsentValidator(t *testing.T) {
	...
	for ; height < keeper.params.SignedBlocksWindow(ctx); height++ {
	...
	require.Equal(t, int64(0), info.StartHeight)
	require.Equal(t, keeper.params.SignedBlocksWindow(ctx), info.SignedBlocksCounter)

	// 500 blocks missed
	for ; height < keeper.params.SignedBlocksWindow(ctx)+(keeper.params.SignedBlocksWindow(ctx)-keeper.params.MinSignedPerWindow(ctx)); height++ {
		ctx = ctx.WithBlockHeight(height)
	...
	require.True(t, found)
	require.Equal(t, int64(0), info.StartHeight)
	require.Equal(t, keeper.params.SignedBlocksWindow(ctx)-keeper.params.MinSignedPerWindow(ctx), info.SignedBlocksCounter)
	...
	require.True(t, found)
	require.Equal(t, int64(0), info.StartHeight)
	require.Equal(t, keeper.params.SignedBlocksWindow(ctx)-keeper.params.MinSignedPerWindow(ctx)-1, info.SignedBlocksCounter)
	...
	ctx = ctx.WithBlockHeader(abci.Header{Time: keeper.params.DowntimeUnbondDuration(ctx) + 1})
	got = slh(ctx, NewMsgUnrevoke(addr))
	require.True(t, got.IsOK())
	...
	// 500 signed blocks
	nextHeight := height + keeper.params.MinSignedPerWindow(ctx) + 1
...
func TestHandleNewValidator(t *testing.T) {
	...
	ctx = ctx.WithBlockHeight(keeper.params.SignedBlocksWindow(ctx) + 1)

	// Now a validator, for two blocks
	keeper.handleValidatorSignature(ctx, val, 100, true)
	ctx = ctx.WithBlockHeight(keeper.params.SignedBlocksWindow(ctx) + 2)
	keeper.handleValidatorSignature(ctx, val, 100, false)

	info, found := keeper.getValidatorSigningInfo(ctx, sdk.ValAddress(val.Address()))
	require.True(t, found)
	require.Equal(t, int64(keeper.params.SignedBlocksWindow(ctx)+1), info.StartHeight)
