...
func (s Store) Get(ctx sdk.Context, key Key, ptr interface{}) error {
	store := ctx.KVStore(s.key)
	bz := store.Get(key.KeyBytes(s.space))
	return s.cdc.UnmarshalBinary(bz, ptr)
}

func (s Store) MustGet(ctx sdk.Context, key Key, ptr interface{}) {
	err := s.Get(ctx, key, ptr)
	if err != nil {
		panic(err)
	}
}
...
func (ros ReadOnlyStore) Get(ctx sdk.Context, key Key, ptr interface{}) error {
